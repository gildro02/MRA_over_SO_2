...
%f_vec=[4,0.1,1,2]; % remember to change 4
f_vec=4;
...
for f_index=1:length(f_vec)
    ...
    for sigma_index=1:length(sigma_vec)
        ...
        M_2_true=2*pi*diag(a_symm_1B)*T*diag(conj(a_symm_1B))+...
            (sigma^2)*(k_symm_1B==k_symm_1B.'|k_symm_1B==-k_symm_1B.');
        ...
        %    +sigma*randn(B+1,N);%N colomns of rotated a vectors
        load('noise.mat');
        a_rot=a_half_1B.*exp(-1i.*k_half_1B.'.*rotations.')... %KEEP -!!!!! though without also works for a
            +squeeze(noise);%N colomns of rotated a vectors
        a_rot_symm=[conj(flip(a_rot(2:end,:),1)) ; a_rot];
        
        M_1_emp=mean(a_rot_symm,2);
        M_2_emp=(1/N)*(a_rot_symm*a_rot_symm');
        M_2_emp_wave = M_2_emp-(sigma^2)*(k_symm_1B==k_symm_1B.'|k_symm_1B==-k_symm_1B.');
        moment_type="emp";
        %moment_type="true";
