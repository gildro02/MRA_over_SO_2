...
N_vec_reduced=ceil(logspace(2,6,10)).';
num_unique_N = length(N_vec_reduced);
num_rep_N = 1;
...
err_squared_FM=zeros(length(sigma_vec),length(f_vec),length(isUniformPowerSpectrum),length(N_vec));
err_squared_Spectral_true=zeros(length(sigma_vec),length(f_vec),length(isUniformPowerSpectrum),length(N_vec));
err_squared_Spectral_emp=zeros(length(sigma_vec),length(f_vec),length(isUniformPowerSpectrum),length(N_vec));
dist_from_circ=zeros(length(f_vec),length(isUniformPowerSpectrum));
bound=zeros(length(f_vec),length(isUniformPowerSpectrum));
a_est_spectral=zeros(length(sigma_vec),length(f_vec),length(isUniformPowerSpectrum),length(N_vec),(2*B+1)*Q);
a_est_FM=zeros(length(sigma_vec),length(f_vec),length(isUniformPowerSpectrum),length(N_vec),(2*B+1)*Q);

theta_min_spectral=zeros(length(sigma_vec),length(f_vec),length(isUniformPowerSpectrum),length(N_vec));
theta_min_FM=zeros(length(sigma_vec),length(f_vec),length(isUniformPowerSpectrum),length(N_vec));
    ...
    for f_index=1:length(f_vec)
        ...
        for sigma_index=1:length(sigma_vec)
            for N_index=1:length(N_vec)
                toc
                disp("f_index="+f_index+", sigma_index="+sigma_index+", UPS="+UPS+", N_index="+N_index)
                ...
                N=N_vec(N_index); %amount of samples
                ...
                [err_squared_FM(sigma_index,f_index,UPS,N_index),theta_min_FM(sigma_index,f_index,UPS,N_index)]=...
                    circ_error_continuous_unrestricted_2D(a_approx_symm_1B_FM,a_symm_1B,Q,B);
                %flip angle so that est rotated is the original.
                %theta_min_FM(sigma_index,f_index,UPS,N_index)=-theta_min_FM(sigma_index,f_index,UPS,N_index);
                %% Spectral Algorithm
                C=BCCB(zeta);
                ...
                a_est_T_true=a_est_tilde_T_true...
                    ...
                    .*sqrt(P_a_true);
                [err_squared_Spectral_true(sigma_index,f_index,UPS,N_index),l_T_true]=...
                ...
                a_est_T_emp=a_est_tilde_T_emp...
                    ...
                    .*sqrt(P_a_emp);
                [err_squared_Spectral_emp(sigma_index,f_index,UPS,N_index),l_T_emp]=...
                    Circ_Error_Continuous_2D(a_est_T_emp,a_symm_1B,Q,B);
                
                theta_min_spectral(sigma_index,f_index,UPS,N_index)=2*pi*l_T_emp/(2*B+1);
                ...
                %for testing
                if err_squared_Spectral_emp(sigma_index,f_index,UPS,N_index)>=5e1
                    %error()
                end
                %% Save resulting estimations, rotated back:
                phase_matrix_FM=repmat(exp(1i.*(-B:B).*theta_min_FM(sigma_index,f_index,UPS,N_index)),[Q,1]);
                phase_matrix_spectral=repmat(exp(1i.*(-B:B).*theta_min_spectral(sigma_index,f_index,UPS,N_index)),[Q,1]);
                ...
                a_est_FM(sigma_index,f_index,UPS,N_index,:)=a_approx_symm_1B_FM_rotated;
                a_est_spectral(sigma_index,f_index,UPS,N_index,:)=a_est_T_emp_rotated ;
...
UPS=1;
f_index=1;
%num_rep_sigma,num_unique_sigma=1
energy_of_a(UPS)=sum(abs(a_symm_1B_cell{UPS}).^2);
err_FM_for_plot=squeeze(mean(reshape(err_squared_FM(...
    :,f_index,UPS,:),num_rep_sigma,num_unique_sigma,num_rep_N,num_unique_N),3))./energy_of_a(UPS);
% vector is 1x1x1xnum_unique_N, need to reshape/squeeze
err_spectral_for_plot=squeeze(mean(reshape(err_squared_Spectral_emp(...
    :,f_index,UPS,:),num_rep_sigma,num_unique_sigma,num_rep_N,num_unique_N),3)).'./energy_of_a(UPS);
figure

lg_cells=cell(1,4);

loglog(N_vec_reduced,err_FM_for_plot,'-blue',"linewidth",1.15);
lg_cells{find(cellfun(@isempty,lg_cells), 1,"first" )}=...
    "$\textnormal{Frequency Marching Algorithm}$";

hold on

loglog(N_vec_reduced,err_spectral_for_plot,'-red',"linewidth",1.15);
lg_cells{find(cellfun(@isempty,lg_cells), 1,"first" )}=...
...
xlabel("$\textnormal{N}$","fontsize",15,"interpreter","latex")
...
for UPS=1:length(isUniformPowerSpectrum)
    ...
    for f_index=1:length(f_indecies_for_plot)
        ...
        err_FM_for_plot=mean(reshape(err_squared_FM(...
            :,f_index,UPS),num_rep_sigma,num_unique_sigma),1).'./energy_of_a(UPS);
...
sigma_index_reduced_image_plot=1;
