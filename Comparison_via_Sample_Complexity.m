...
sigma_squared_over_N = 1e-7;
num_unique_N = 75;
num_rep = 5000;
N_vec_reduced = ceil(logspace(0, 6, num_unique_N).');
N_vec = repelem(N_vec_reduced, num_rep);
% Make sure that the ratio between the SNR and N is constant.
sigma_vec_reduced = sqrt(sigma_squared_over_N .* N_vec_reduced);
...
for n = 1:length(N_vec)
    [error_squared_FM(n), coeff_FM, error_squared_spectral(n), coeff_spectral, M_1_error(n), M_2_error(n)] = ...
        runNumericalExperiment('B', B, 'Q', Q, 'N', N_vec(n), 'sigma', sigma_vec(n), 'rho_func',...
...
fig = plotErrorAsFunctionOfX(N_vec, "N", error_squared_spectral_relative,...
    error_squared_FM_relative, num_rep, dist_from_circ);

%% Save the plot:
% Push test_number up by 1, make directory of test
dir_path = makeNewTestDir();
notes = "Insuring a constant sample complexity";
figure_name = "const_sample_complexity_circulant.fig";
saveFigureAndNotes(fig, figure_name, dir_path, notes);
sizeThresholdKB = 4000; %CHECK BEFORE LARGE RUN!!!!
