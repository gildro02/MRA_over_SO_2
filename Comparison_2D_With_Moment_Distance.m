...
%sigma_vec_reduced=logspace(-5,0,50).';

%get a sigma_vec_reduced with a lower concentration of values at the start
%than at the end
...
num_unique_sigma = 75;
step_func = @(x) x.^(1/2.5);
step_vector = step_func(linspace(0, 1, num_unique_sigma).');
%step_vector = [0; cumsum(seed_vector./sum(seed_vector))];
sigma_vec_reduced = 10.^(min_sigma_power + (max_sigma_power-min_sigma_power).*step_vector);

%sigma_vec_reduced=logspace(-3,0,20).';
...
M_1_squared_error = zeros(length(sigma_vec),length(f_vec),length(isUniformPowerSpectrum)); %squared
M_2_squared_error = zeros(length(sigma_vec),length(f_vec),length(isUniformPowerSpectrum)); %squared
...
for UPS=1:length(isUniformPowerSpectrum)
    ...
    for f_index=1:length(f_vec)
        ...
        for sigma_index=1:length(sigma_vec)
            ...
            M_1_squared_error(sigma_index, f_index, UPS) = norm(M_1_true-M_1_emp,"fro").^2; %squared
            %second moment
            ...
            M_2_squared_error(sigma_index, f_index, UPS) = norm(M_2_true-M_2_emp,"fro").^2; %squared
...
for UPS=1:length(isUniformPowerSpectrum)
    ...
    for f_index=1:length(f_vec)
        ...
        %this is slightly incorrect - using the mean on all moment errors
        %instead of plotting each one individually
        [M_2_error_squared_for_plot, I_for_plot] = sort(mean(reshape(M_2_squared_error(...
            :,f_index,UPS),num_rep,num_unique_sigma),1));
        ...
        loglog(M_2_error_squared_for_plot, err_FM_for_plot(I_for_plot), '-blue', "linewidth", 1.15);
        ...
        loglog(M_2_error_squared_for_plot, err_spectral_for_plot(I_for_plot), '-red', "linewidth", 1.15);
        lg_cells{find(cellfun(@isempty,lg_cells), 1,"first" )}=...
            "$\textnormal{Spectral Algorithm}$";
        
        %linear fit to the logarithm:
        if f_vec(f_index) == 0
            %M_2_error_squared_for_plot is a row, needs to be a column - that
            %why we transpose
            % take all the measurements exept the last 10 so that the fit
            % is more accurate at the start.
            [linear_fit, gof] = fit(log10(M_2_error_squared_for_plot(1:end-10).'),...
                log10(err_spectral_for_plot(I_for_plot(1:end-10))), "poly1");
            linear_fit_func = @(x) 10.^(linear_fit.p2) .* x.^(linear_fit.p1);
            loglog(M_2_error_squared_for_plot, linear_fit_func(M_2_error_squared_for_plot), '--magenta', "linewidth", 1);
            lg_cells{find(cellfun(@isempty,lg_cells), 1,"first" )} = "$y=" + round((10.^linear_fit.p2), 2) + ...
                "x^{" + round(linear_fit.p1, 2) + "},\:R^2=" + round(gof.rsquare, 2) + "$";
        end
        ...
        xlabel("$\textnormal{Squared Error of } M_2$", "fontsize", 15, "interpreter", "latex")
...
%save all relavent (numeric) variables:
folderPath = strcat('./Figures_Thesis/Comparison_2D_Archive/', test_end_string);
fileName = 'all_numeric_variables';
sizeThresholdKB = 10000;
saveNumericVariablesBelowThreshold(folderPath, fileName, sizeThresholdKB)

sigma_index_reduced_image_plot=length(sigma_vec_reduced);
